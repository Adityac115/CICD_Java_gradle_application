            ...
            steps{
                script{
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                             sh '''
                                docker build -t 34.100.130.110:8083/springapp:${VERSION} .
                                docker login -u admin -p $docker_password 34.100.130.110:8083 
                                docker push  34.100.130.110:8083/springapp:${VERSION}
                                docker rmi 34.100.130.110:8083/springapp:${VERSION}
                            '''
                    }
                }
            }
                    ...
                    withCredentials([string(credentialsId: 'docker_pass', variable: 'docker_password')]) {
                          dir('kubernetes/') {
                             sh '''
                                 ...
                                 curl -u admin:$docker_password http://34.100.130.110:8081/repository/helm-hosted/ --upload-file myapp-${helmversion}.tgz -v
                            '''
                          }
                    }
                }
